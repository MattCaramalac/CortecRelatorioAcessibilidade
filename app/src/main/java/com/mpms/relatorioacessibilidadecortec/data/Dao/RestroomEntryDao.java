...
public interface RestroomEntryDao {
    ...
    @Query("SELECT restroomID, blockID, restType, restGender, restLocation, collectiveHasDoor, entranceWidth, entranceDoorSill, entranceDoorSillObs, " +
            ...
            "winComHeight3, restFirstPhoto FROM RestroomEntry WHERE restroomID = :restID")
    ...
    @Query("SELECT restroomID, blockID, restType, upViewLength, upViewWidth, upViewMeasureA, upViewMeasureB, upViewMeasureC, upViewMeasureD, upViewMeasureD, upViewObs, restUpperPhoto" +
    ...
    @Query("SELECT restroomID, blockID, restType, toType, toHeightNoSeat, toHasSeat, toHeightSeat, toHasSoculo, frSoculo, latSoculo, socCorners, toHasFrontBar, frBarA, frBarB, frBarC, frBarSect, " +
            ...
            "papEmbHeight, papSupAlign, papSupHeight, papHoldObs, hasDouche, douchePressHeight, doucheActHeight, doucheObs, toiletObs, restToiletPhoto FROM RestroomEntry " +
    ...
    @Query("SELECT restroomID, blockID, restType, hasHanger, hangerHeight, hangerObs, hasObjHold, objHoldCorrect, objHoldHeight, objHoldObs, hasSoapHold, soapHoldHeight, soapHoldObs," +
            "hasTowelHold, towelHoldHeight, towelHoldObs, restAccessPhoto FROM RestroomEntry WHERE restroomID = :restID")
    LiveData<RestroomEntry> getRestAccessData(int restID);

    @Query("SELECT restroomID, blockID, restType, hasEmergencyButton, emergencyHeight, emergencyObs, hasWaterValve, waterValveType, waterValveHeight, waterValveObs, hasWindow, winQnt, " +
            "winComType1, winComHeight1, winComType2,winComHeight2, winComType3, winComHeight3, hasWallMirror, wallMirrorLow, wallMirrorHigh, wallMirrorObs,restAccessPhoto2 FROM " +
            "RestroomEntry WHERE restroomID = :restID")
    ...
    @Query("SELECT restroomID,  blockID, restType, hasSink, sinkType, approxMeasureA, approxMeasureB, approxMeasureC, approxMeasureD, approxMeasureE, hasSinkBar, hasLeftFrontHorBar, leftFrontHorMeasureA, leftFrontHorMeasureB, " +
            ...
            "restSinkPhoto FROM RestroomEntry WHERE restroomID = :restID")
    LiveData<RestroomEntry> getRestSinkData(int restID);

    @Query("SELECT restroomID,  blockID, restType, approxMeasureB, approxMeasureC, hasSinkBar, sinkObs, hasLowerSink, restSinkPhoto " +
            "FROM RestroomEntry WHERE restroomID = :restID")
    ...
    @Query("SELECT restroomID, blockID, hasUrinal, restType, hasAccessUrinal, urinalType, urMeasureA, urMeasureB, urMeasureC, urMeasureD, urMeasureE, urMeasureF, urMeasureG, " +
            "urMeasureH, urMeasureI, urMeasureJ, urMeasureK, urMeasureL, urMeasureM, urObs, restUrinalPhoto FROM RestroomEntry WHERE restroomID = :restID")
